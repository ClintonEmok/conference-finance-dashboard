...
it("renders each fixed kind with canonical values and supplied link", async () => {
  for (const kind of ["unpaid", "partial", "overdue"] as const) {
    ...
    expect(html).toContain(PAYMENT_REMINDER_COPY[kind].subject); expect(html).toContain(PAYMENT_REMINDER_COPY[kind].message); expect(html).toContain("Spring Conference"); expect(html).toContain("BK-1"); expect(html).toContain("https://example.test/booking/BK-1/manage")
  }
})
