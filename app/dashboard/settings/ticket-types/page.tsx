...
export default function TicketTypesSettingsPage() {
  ...
  useEffect(() => {
    async function loadEvents() {
      ...
      try {
        const response = await fetch("/api/dashboard/attendees?page=1&pageSize=1")
        if (!response.ok) {
          throw new Error("Failed to load events")
        }
        const body = (await response.json()) as { availableEvents: EventOption[] }
  ...
  useEffect(() => {
    ...
    async function loadTemplates() {
      ...
      try {
        const response = await fetch(`/api/dashboard/tikkie-templates?eventId=${encodeURIComponent(selectedEventId)}&summary=1`)
        if (!response.ok) {
  ...
  async function handleSubmitForm(e: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch("/api/dashboard/tikkie-templates", {
        ...
        body: JSON.stringify(editingTemplateId ? { ...payload, id: editingTemplateId } : payload),
      })

      if (!response.ok) {
        const body = (await response.json()) as { error?: { message?: string } } | null
        throw new Error(body?.error?.message ?? `Failed to ${editingTemplateId ? "update" : "create"} template`)
      }

      closeForm()
      // Reload templates
      const refreshResponse = await fetch(`/api/dashboard/tikkie-templates?eventId=${encodeURIComponent(selectedEventId)}&summary=1`)
      if (refreshResponse.ok) {
        const body = (await refreshResponse.json()) as { templates: TemplateSummary[] }
        setTemplates(body.templates)
      }
    } catch (err) {
  ...
  async function handleDeleteTemplate(templateId: string) {
    if (!confirm("Are you sure you want to remove this template? It will be soft-deleted and no longer used for auto-fill.")) {
      return
    }

    try {
      const response = await fetch(`/api/dashboard/tikkie-templates?id=${encodeURIComponent(templateId)}`, {
        method: "DELETE",
      })
    ...
    <section className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Configure reusable Tikkie payment templates per ticket type. When generating links, the amount and description will be pre-filled from the matching template.
        </p>
      </header>
      ...
      {selectedEventId && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Each unique ticket type for the selected event is shown below. Configure a template to enable auto-fill when generating Tikkie links.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoadingTemplates ? (
              ...
              <p className="text-sm text-muted-foreground">
                No ticket types found for this event. Ticket types are discovered from attendee records.
              </p>
            ) : (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      {item.template ? (
                        <div className="mt-2 space-y-1 text-sm text-muted-foreground">
                          <p>
                            <span className="font-medium text-foreground">{formatMoney(item.template.amountMinor)}</span>
                            {" · "}
                            Expires in {item.template.expiryDays} days
                          </p>
                          <p className="truncate">{item.template.descriptionTemplate}</p>
                        </div>
                      ) : (
                        <p className="mt-2 text-sm text-muted-foreground">
                          No template configured. Tikkie links will use default values.
                        </p>
                      )}
                    </div>
                    <div className="flex shrink-0 gap-2">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          item.template ? void openEditForm(item.template) : void openAddForm(item.ticketTypeLabel)
                        }
                      >
                        {item.template ? (
                          <>
                      ...
                      {item.template && (
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => void handleDeleteTemplate(item.template!.id)}
                        >
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 px-4 py-8 backdrop-blur-sm">
          <div className="absolute inset-0" onClick={() => !isSubmitting && closeForm()} />
          <Card className="relative z-10 w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between border-b border-border/70 bg-muted/30 px-5 py-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{formState.ticketTypeLabel}</p>
              </div>
              <Button
                type="button"
                variant="ghost"
              ...
              <form onSubmit={(e) => void handleSubmitForm(e)} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormState((s) => ({ ...s, amountMinor: e.target.value }))}
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormState((s) => ({ ...s, descriptionTemplate: e.target.value }))}
                  ...
                  <p className="text-xs text-muted-foreground">
                    Supports placeholders: {"{{name}}"}, {"{{email}}"}, {"{{ticketType}}"}
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormState((s) => ({ ...s, expiryDays: e.target.value }))}
                    placeholder="14"
                    className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                  />
                  <p className="text-xs text-muted-foreground">
