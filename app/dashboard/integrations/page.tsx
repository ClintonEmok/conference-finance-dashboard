...
import { getIntegrationStatus, type IntegrationStatusRecord } from "@/lib/integrations/status"
...
> = {
  configured: {
    label: "Configured",
    className: "border-green-200 bg-green-50 text-green-700 dark:border-green-900 dark:bg-green-950/20 dark:text-green-300",
  ...
  unreachable: {
    ...
    action: "Validate provider availability, credentials, and outbound network access.",
  },
}
...
export default async function IntegrationsPage() {
  ...
  if (!session) {
    redirect("/login?callbackUrl=%2Fdashboard%2Fintegrations")
  }
  ...
  return (
    <section className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-xs text-muted-foreground">Last checked: {status.generatedAt}</p>
      </header>
      ...
      <div className="grid gap-4">
        {status.providers.map((provider) => {
          ...
          return (
            <article key={provider.provider} className="rounded-lg border border-border bg-card p-5 shadow-sm">
              ...
              <p className="mt-3 text-sm text-muted-foreground">{state.action}</p>
              ...
              <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
                {Object.entries(provider.diagnostics).map(([key, value]) => (
                  <div key={key} className="rounded-md border border-border/70 p-2">
                    ...
                    <dd className="mt-1 font-mono text-xs">{renderDiagnosticValue(value)}</dd>
                  </div>
                ))}
              </dl>
              ...
              <div className="mt-4 rounded-md border border-border/70 p-3 text-sm">
                ...
                <p className="mt-1 text-muted-foreground">{provider.connectivity.message}</p>
