...
export function TikkieLinkSummary({
  ...
  const description = useMemo(
    () => latestLink?.description?.trim() || "Operator follow-up link",
    [latestLink]
  )
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-2xl border border-slate-200/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(245,247,251,0.98))] p-4 shadow-sm dark:border-slate-800 dark:bg-[linear-gradient(180deg,rgba(15,23,42,0.9),rgba(17,24,39,0.9))]">
        ...
        >
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <span
                className={cn(
                  "inline-flex rounded-full border px-2.5 py-1 text-[11px] font-semibold tracking-[0.16em] uppercase",
                  statusClass(latestLink.status)
                )}
              >
                {statusLabel(latestLink.status)}
              </span>
              {latestLink.checkState === "stale" && (
                <span className="inline-flex rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-[11px] font-semibold tracking-[0.16em] text-amber-700 uppercase dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-300">
                  Status check stale
                </span>
              )}
            </div>
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                {formatMoney(latestLink.amountMinor)} • Token{" "}
                {latestLink.paymentRequestToken}
              </p>
            </div>
          </div>
          ...
          <div className={cn("flex flex-wrap gap-2", compact && "w-full")}>
            <Button
              ...
              size="sm"
              disabled={isCopying}
              onClick={() => onCopy?.(latestLink.paymentRequestUrl)}
            >
            ...
            <Button type="button" variant="outline" size="sm" asChild>
              <a
                href={latestLink.paymentRequestUrl}
                target="_blank"
                rel="noreferrer"
              >
        ...
        <div
          className={cn(
            "mt-4 grid gap-3 text-xs text-slate-500 dark:text-slate-400",
        ...
        >
          <div>
            ...
            <p className="mt-1 text-sm text-slate-700 dark:text-slate-200">
              {formatDateTime(latestLink.providerLastCheckedAt)}
            </p>
          </div>
          <div>
            <p className="font-semibold tracking-[0.16em] uppercase">Source</p>
            <p className="mt-1 text-sm text-slate-700 dark:text-slate-200">
              {sourceLabel(latestLink.statusSource)}
            </p>
          </div>
          <div>
            <p className="font-semibold tracking-[0.16em] uppercase">Expires</p>
            <p className="mt-1 text-sm text-slate-700 dark:text-slate-200">
              {formatDateTime(latestLink.expiryDate)}
            </p>
          </div>
          <div>
            <p className="font-semibold tracking-[0.16em] uppercase">Updated</p>
            <p className="mt-1 text-sm text-slate-700 dark:text-slate-200">
              {formatDateTime(latestLink.statusUpdatedAt)}
            </p>
          </div>
        </div>
      </div>
          ...
          >
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Keep the latest link in view and expand older links only when
                needed.
              </p>
            </div>
            <span className="inline-flex items-center gap-2 text-xs font-semibold tracking-[0.16em] text-slate-500 uppercase dark:text-slate-400">
          ...
          {showHistory && (
            <div className="space-y-3 border-t border-slate-200/80 px-4 py-4 dark:border-slate-800">
              {history.map((link) => (
                <div
                  key={link.id}
                  className="rounded-xl border border-slate-200/80 bg-slate-50/70 px-3 py-3 dark:border-slate-800 dark:bg-slate-900/60"
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                        {formatMoney(link.amountMinor)} • Token{" "}
                        {link.paymentRequestToken}
                      </p>
                    </div>
                    <span
                      className={cn(
                        "inline-flex rounded-full border px-2 py-0.5 text-[11px] font-semibold tracking-[0.16em] uppercase",
