...
test("authenticated attendee ledger searches joined fields and uses event all-time normalization", async () => {
  ...
  await t.mutation(async (ctx) => { const { upsertAttendeeSearchDocument } = await import("./search"); await upsertAttendeeSearchDocument(ctx, ids.attendee) })
  const result = await t.query(api.attendees.getAttendeeLedgerPage, { eventId: ids.event, search: "Search Family", pageSize: 10, from: null, to: null, cursor: null })
