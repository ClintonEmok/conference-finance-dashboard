...
describe("manual Allocation workflow source contract", () => {
  ...
  it("keeps preview separate from the assignment mutation", () => {
    ...
    expect(source).toContain("Parent placement required")
    expect(source).toContain("Needs family link")
    expect(source).toContain("Waiting for parent room")
    expect(source).toContain("No bed required · follows parent")
