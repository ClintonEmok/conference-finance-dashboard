...
describe("accommodation route compatibility", () => {
  ...
  it("preserves workspace tab and room intent", async () => {
    ...
    expect(mocks.redirect).toHaveBeenCalledWith("/dashboard/events/spring%20retreat/accommodation?tab=allocation&roomId=room%2F7")
  })

  it("preserves allocation and room redirects with encoded values", async () => {
    await LegacyAllocationPage({ params: Promise.resolve({ slug: "event/one" }), searchParams: Promise.resolve({ roomId: "room/7" }) })
    expect(mocks.redirect).toHaveBeenCalledWith("/dashboard/events/event%2Fone/accommodation?tab=allocation&roomId=room%2F7")
    mocks.redirect.mockReset()
  ...
  it("preserves the canonical workspace and disables root-level event surfaces", () => {
    ...
    expect(workspace).toContain('label: "Allocation"')
