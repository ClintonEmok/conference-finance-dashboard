...
describe("Ticket Tailor client integration", () => {
  ...
  it("builds Basic auth and returns JSON payload", async () => {
    ...
    const payload = await ticketTailorFetch<{ data: Array<{ id: string }> }>("/events")
    ...
    expect(authHeader).toBe("Basic c2tfdGVzdF8xMjM0NTY3ODkwMTIzNDU2Og==")
  })
  ...
  it("falls back to /orders when nested event orders endpoint is unavailable", async () => {
    ...
    expect(String(fetchMock.mock.calls[0][0])).toContain("/events/ev_target/orders")
    expect(String(fetchMock.mock.calls[1][0])).toContain("/orders")
    expect(result.items).toEqual([{ id: "ord_1", event_summary: { id: "ev_target" } }])
  })
